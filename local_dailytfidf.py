...
import csv
with open('datatrain.csv','r',encoding='utf-8')as datatrain:
    semesta=csv.reader(datatrain)
    for ai in semesta:
        if ai[1] not in dbtanggal:
            dbtanggal.append(ai[1])
            
with open('stopword.csv','r')as listopword:
    stopword=csv.reader(listopword)
